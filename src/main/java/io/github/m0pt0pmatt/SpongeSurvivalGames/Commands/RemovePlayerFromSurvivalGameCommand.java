 ...
 */
public class RemovePlayerFromSurvivalGameCommand implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        if (!plugin.getSurvivalGameMap().get(id.get()).getGameState().equals(SurvivalGameState.READY)){
            plugin.getLogger().error("Survival Game \"" + id.get() + "\" must be READY to remove a player.");
            return CommandResult.empty();
        }
        ...
        plugin.getSurvivalGameMap().get(id.get()).removePlayer(player.get().getUniqueId());
        plugin.getLogger().info("Player \"" + playerName.get() + "\" removed from survival game \"" + id.get() + "\".");
